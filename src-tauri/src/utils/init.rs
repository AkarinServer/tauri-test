...
async fn initialize_config_files() -> Result<()> {
        ...
        && !path.exists()
    {
        ...
        help::save_yaml(&path, &template, Some("# RV Verge"))
            .await
...
pub fn init_scheme() -> Result<()> {
    ...
    clash.set_value("", &"RV Verge")?;
    clash.set_value("URL Protocol", &"RV Verge URL Scheme Protocol")?;
...
#[cfg(target_os = "linux")]
pub fn init_scheme() -> Result<()> {
    const DESKTOP_FILE: &str = "rv-verge.desktop";
...
#[cfg(target_os = "linux")]
const DEEP_LINK_SCHEMES: &[&str] = &["clash", "rv-verge"];
