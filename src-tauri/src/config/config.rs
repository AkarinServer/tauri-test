...
impl Config {
    ...
    pub async fn generate_file(typ: ConfigType) -> Result<PathBuf> {
        logging!(info, Type::Config, "[Config::generate_file] 开始生成配置文件，类型: {:?}", typ);
        
        logging!(info, Type::Config, "[Config::generate_file] 步骤1: 获取应用主目录");
        let app_home = match dirs::app_home_dir() {
            Ok(dir) => {
                logging!(info, Type::Config, "[Config::generate_file] 应用主目录: {:?}", dir);
                dir
            }
            Err(e) => {
                logging!(error, Type::Config, "[Config::generate_file] 获取应用主目录失败: {}", e);
                logging!(error, Type::Config, "[Config::generate_file] 错误详情: {:#}", e);
                return Err(e);
            }
        };
        
        let path = match typ {
            ConfigType::Run => app_home.join(files::RUNTIME_CONFIG),
            ConfigType::Check => app_home.join(files::CHECK_CONFIG),
        };
        logging!(info, Type::Config, "[Config::generate_file] 配置文件路径: {:?}", path);

        logging!(info, Type::Config, "[Config::generate_file] 步骤2: 获取运行时配置");
        let runtime = Self::runtime().await;
        let runtime_arc = runtime.latest_arc();
        let config = match runtime_arc.config.as_ref() {
            Some(cfg) => {
                logging!(info, Type::Config, "[Config::generate_file] 运行时配置获取成功");
                cfg
            }
            None => {
                logging!(error, Type::Config, "[Config::generate_file] 运行时配置为空");
                logging!(error, Type::Config, "[Config::generate_file] 可能原因: 配置尚未生成或初始化失败");
                return Err(anyhow!("failed to get runtime config"));
            }
        };

        logging!(info, Type::Config, "[Config::generate_file] 步骤3: 保存配置文件到磁盘");
        match help::save_yaml(&path, config, Some("# Generated by RV Verge")).await {
            Ok(_) => {
                logging!(info, Type::Config, "[Config::generate_file] 配置文件保存成功: {:?}", path);
                Ok(path)
            }
            Err(e) => {
                logging!(error, Type::Config, "[Config::generate_file] 配置文件保存失败: {}", e);
                logging!(error, Type::Config, "[Config::generate_file] 错误详情: {:#}", e);
                logging!(error, Type::Config, "[Config::generate_file] 可能原因: 磁盘空间不足、权限不足或路径无效");
                Err(e)
            }
        }
    }
    ...
    pub async fn verify_config_initialization() {
        ...
        let backoff_strategy = ExponentialBackoff {
            initial_interval: std::time::Duration::from_millis(50), // 从 100ms 减少到 50ms
            max_interval: std::time::Duration::from_secs(1), // 从 2s 减少到 1s
            max_elapsed_time: Some(std::time::Duration::from_secs(3)), // 从 10s 减少到 3s
            multiplier: 2.0,
            ..Default::default()
        };
